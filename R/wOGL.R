...
wOGL <- function( x,
  ...
  for(i in 1:nfold){
    ...
    which <- foldid == i
    x.tran <- x[!which, ]; y.tran <- y[!which]
    x.vald <- x[which, ];  y.vald <- y[which]
  ...
  opt.lambda <- lambdas[which.min(mats)]
  sgnc <- glmnet(x, y, family="binomial", lambda=opt.lambda)$beta
  if (!is.null(sgnc)) {
    sgnc <- sign(sgnc)
    if (length(sgnc) != ncol(x)) stop("the length of sgnc should be equal to p.")
  }

  g1 <- pclogit(x, y, stra=NULL, alpha=alpha, group=adjm, lambda=opt.lambda, sgnc=sgnc)
  g2 <- per.gglasso(x, y, group=group, nperm=nperm)
  ogl.sp <- g2$per.sp.mean


  wj <- tapply(g1$beta[unlist(group)], gr.new, function(x) sqrt(sum(x^2)))
  wjs <- wj/sum(wj)
  if (sum(is.na(wjs)) != 0) {
    wjs[is.na(wjs)] <- 0
  } else {
